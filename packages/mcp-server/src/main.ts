...
import { buildNodeContext, comparePng, composeBundleVectorGroupSvg, resolveNodeReference, searchNodes, type AgentDocument } from '@figctx/core';
import { z } from 'zod';
...
server.registerTool('get_style_tokens', { description: 'Read extracted token files, font requirements, and Figma variables when present in the local export.' }, async () => text({ ...Object.fromEntries(tokenFiles), fonts: fontFile, variables: variableFile }));
