...
server.registerTool(publicToolNames.listFrames, { description: 'List locally extracted frames and canvases in extracted order.', inputSchema: publicToolSchemas.list_frames }, async ({ cursor, limit }) => { const page = pageFrames(document, { cursor, limit }); return text({ ...page, items: page.items.map((node) => presentNode(node, document)) }); });
