...
import { buildNodeContext, comparePng, composeBundleVectorGroupSvg, effectiveChildIds, inspectNode, resolveNodeReference, searchNodes, type AgentDocument, type AgentNode } from '@figctx/core';
import { z } from 'zod';
...
import { resultBundle, resultText } from './result.js';
...
const resultFileHint = ' When this tool returns a saved result path, immediately read that absolute path with the filesystem read tool. Parse content[0].text as JSON when the file contains an MCP response envelope.';
const text = (toolName: string, value: unknown) => resultText(toolName, value);
server.registerTool(publicToolNames.listFrames, { description: `List locally extracted frames and canvases in extracted order.${resultFileHint}`, inputSchema: publicToolSchemas.list_frames }, async ({ cursor, limit }) => { const page = pageFrames(document, { cursor, limit }); return text(publicToolNames.listFrames, { ...page, items: page.items.map((node) => presentNode(node, document)) }); });
server.registerTool(publicToolNames.listFrameSummaries, { description: `List compact frame and canvas summaries in sequential batches.${resultFileHint}`, inputSchema: publicToolSchemas.list_frame_summaries }, async ({ cursor, limit }) => text(publicToolNames.listFrameSummaries, pageFrameSummaries(document, new Set(referenceIndex.references.map((reference) => reference.nodeId)), { cursor, limit })));
server.registerTool(publicToolNames.searchNodes, { description: 'Search local nodes by case-insensitive name or text substring.', inputSchema: publicToolSchemas.search_nodes }, async ({ query, type, limit }) => text(publicToolNames.searchNodes, searchNodes(document, query, { type, limit })));
server.registerTool(publicToolNames.getNodeContext, { description: `Resolve a local node ID or Figma URL, including an attached reference PNG if available.${resultFileHint}`, inputSchema: publicToolSchemas.get_node_context }, async ({ reference }) => { const node = resolveNodeReference(document, reference); return text(publicToolNames.getNodeContext, { node: presentNode(node, document), reference: referenceIndex.references.find((item) => item.nodeId === node.id) }); });
server.registerTool(publicToolNames.getFrameBundle, { description: `Return complete local subtree context, including descendant text, assets, vectors, style tokens, and attached reference PNGs.${resultFileHint}`, inputSchema: publicToolSchemas.get_frame_bundle }, async ({ reference }) => { const context = buildNodeContext(document, resolveNodeReference(document, reference)); return text(publicToolNames.getFrameBundle, { ...context, node: presentNode(context.node, document), nodes: context.nodes.map((node) => presentNode(node, document)), tokens: tokensFor(context.nodeIds), references: referenceIndex.references.filter((item) => context.nodeIds.includes(item.nodeId)), visualBaseline: manifest.visualBaseline }); });
server.registerTool('review_visual_match', { description: 'Call after the first implementation screenshot and again before completion. Compares an attached Figma reference PNG with candidatePath, returns reference/candidate/diff images and a corrective prompt. When passed is false, revise and call this tool again before declaring completion.', inputSchema: { reference: z.string(), candidatePath: z.string(), phase: z.enum(['midpoint', 'final']) } }, async ({ reference, candidatePath, phase }) => {
  ...
  return resultBundle('review_visual_match', { content: buildVisualReview({ nodeId: node.id, phase: phase as ReviewPhase, reference: referencePng, candidate: candidatePng, comparison: comparePng(referencePng, candidatePng) }).content });
});
server.registerTool(publicToolNames.getVectorSvg, { description: 'Compose one listed vector-only group into a self-contained SVG without modifying the bundle.', inputSchema: publicToolSchemas.get_vector_svg }, async ({ reference }) => { const node = resolveNodeReference(document, reference); const svg = await composeBundleVectorGroupSvg(bundleRoot, document, node.id); if (!svg) throw new Error(`No renderable vector group matches ${reference}`); return text(publicToolNames.getVectorSvg, { nodeId: node.id, svg }); });
server.registerTool(publicToolNames.getStyleTokens, { description: 'Read extracted token files, font requirements, and Figma variables when present in the local export.' }, async () => text(publicToolNames.getStyleTokens, { ...Object.fromEntries(tokenFiles), fonts: fontFile, variables: variableFile }));
server.registerTool(publicToolNames.getAsset, { description: 'Return the local extracted image path by hash.', inputSchema: publicToolSchemas.get_asset }, async ({ hash }) => {
  ...
  return text(publicToolNames.getAsset, { hash, path: `assets/images/${entry}`, absolutePath: join(bundleRoot, 'assets/images', entry) });
});
server.registerTool(publicToolNames.inspectNode, { description: 'Return a bounded local node summary without asset paths or hashes.', inputSchema: publicToolSchemas.inspect_node }, async ({ reference, depth, maxChildren }) => text(publicToolNames.inspectNode, inspectNode(document, resolveNodeReference(document, reference), { depth, maxChildren })));
...
function presentNode(node: AgentNode, _document: AgentDocument): AgentNode & { effectiveChildIds: string[] } {
  const children = effectiveChildIds(node);
  return { ...node, childIds: children, effectiveChildIds: children };
}
