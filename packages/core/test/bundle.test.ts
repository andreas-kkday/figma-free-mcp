...
test('writes an atomic inspectable bundle', async () => {
  ...
  await writeBundle({ outDir, manifest: { contractVersion: '1', status: 'success' }, raw: { source: 'test' }, agent: { contractVersion: '1', rootIds: [], nodesById: {} }, images: [{ hash: 'a', bytes: Uint8Array.from([0xff, 0xd8, 0xff]), format: 'jpeg' }], vectors: [{ blobId: 7, bytes: Uint8Array.from([5, 6]) }], thumbnail: Uint8Array.from([1, 2, 3]), tokens });
